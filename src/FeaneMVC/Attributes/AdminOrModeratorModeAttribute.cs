...
{
    public class AdminOrModeratorModeAttribute : ActionFilterAttribute
    {
        private readonly ISessionService _session;

        // Constructor to initialize the ISession instance
        public AdminOrModeratorModeAttribute(ISessionService session)
        {
            _session = session;
        }
        ...
        public override void OnActionExecuting(ActionExecutingContext context)
        {
            ...
            if (apiCookie != null)
            {
                ...
                // Check if the profile is not null and if the user role is either Admin or Moderator
                if (profile != null && (profile.Roles == Role.Admin || profile.Roles == Role.Moderator))
                {
                ...
                {
                    // Redirect to the error page if the user is not an Admin or Moderator
