...
Микросервис `ProductService` отвечает за каталог блюд и публичное меню. Он предоставляет REST API с поддержкой загрузки изображений (в base64), аналитикой и гибкой фильтрацией:

- `GET /api/dishes` — список блюд с фильтрами по категории, поиску, доступности и сортировке.
- `GET /api/dishes/{id}` — получение одного блюда.
- `POST /api/dishes` — создание блюда (multipart/form-data, поддерживается загрузка изображения).
- `POST|PUT /api/dishes/{id}` — обновление существующего блюда.
- `DELETE /api/dishes/{id}` — удаление блюда.
- `GET /api/dishes/categories` — набор доступных категорий.
- `GET /api/menu` — короткий список позиций меню для фронтенда.
- `GET /api/analytics/summary` — агрегированные метрики по меню (кол-во блюд, средняя цена, популярные категории).

Сервис контейнеризован, автоматически применяет EF Core миграции и поставляется с начальными данными меню.
