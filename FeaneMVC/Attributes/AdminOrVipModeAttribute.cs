...
{
    public class AdminOrVipModeAttribute : ActionFilterAttribute
    {
        private readonly ISessionService _session;

        // Constructor to initialize the ISession instance
        public AdminOrVipModeAttribute(ISessionService session)
        {
            _session = session;
        }
        ...
        public override void OnActionExecuting(ActionExecutingContext context)
        {
            ...
            if (!string.IsNullOrEmpty(apiCookie))
            {
                ...
                // Check if the profile is not null and if the user role is either Admin or VIP
                if (profile != null && (profile.Roles == Role.Admin || profile.Roles == Role.VIP))
                {
                ...
                {
                    // Redirect to the error page if the user is not an Admin or VIP
